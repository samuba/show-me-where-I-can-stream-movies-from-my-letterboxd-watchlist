<script lang="ts">
    ...
    if (browser) {
      ...
      letterboxdLists = [{ name: "dummy", entries: [], streamProviders: [], url: "", description: ""}]
    }

    const selectedListNameFromLocalstorage = browser ? letterboxdLists.find(x => x.name === localStorage.getItem("selectedList")) ? localStorage.getItem("selectedList") : undefined : undefined
...
<div class="container" style="margin-top: 3rem">
    ...
    <select id="lists-select" bind:value={selectedListName} on:change={(e) => saveSelectedList(e?.target?.value)}>
      ...
      <div style="margin-bottom: 3rem">
          {#each selectedList.streamProviders as provider}
            <div style="display: inline-block">
              <input bind:checked={provider.enabled} on:change={(e) => saveProviderEnabled(provider.name, e?.target?.checked)} type="checkbox" id={provider.name} />
              <label
                for={provider.name}
                title={provider.name}
    ...
    <div style="margin-top: 3rem; ">
      ...
      <a href={selectedList.url} target="_blank" rel="noreferrer">
        <svg xmlns="http://www.w3.org/2000/svg"  style="height: 1.1rem; width: 1.1rem;" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
